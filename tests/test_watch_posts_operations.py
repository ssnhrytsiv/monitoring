...
def test_create_watch_and_get():
    _reset_db()
    wid = wp_db.create_watch(channel_id=101, template_id=10, source_url="https://t.me/+abc")
    data = wp_db.get_watch_by_id(wid)
...
def test_update_status_and_time_window():
    _reset_db()
    wid = wp_db.create_watch(channel_id=202, time_window_end="2026-01-01 10:00")
    ok = wp_db.set_watch_status_pending_db(wid)
    assert ok is True
    ok = wp_db.update_watch_time_window(wid, "2026-01-01 09:00", "2026-01-01 10:30")
    assert ok is True
    _, _, status, tw_end, _, _, _ = wp_db.get_watch_by_id(wid)
    assert status == "pending"
...
def test_group_leader_and_load_group_items():
    _reset_db()
    wid1 = wp_db.create_watch(channel_id=1, template_id=55, time_window_end="2026-01-02 12:00", created_by=5)
    wid2 = wp_db.create_watch(channel_id=2, template_id=55, time_window_end="2026-01-02 12:00", created_by=5)
    leader = wp_db.get_group_leader_for_watch(wid2)
    tpl, tw_key, created_by, ch = wp_db.get_group_leader_key(wid2)
    items = wp_db.load_group_items(template_id=55, tw_key="2026-01-02 12:00", created_by=5)
    assert leader == wid1
...
def test_watch_events_insert_and_mark_sent():
    _reset_db()
    wid = wp_db.create_watch(channel_id=303)
    payload = json.dumps({"watch_id": wid})
    we_db.insert_watch_event(wid, "matched", payload)
    events = we_db.fetch_unsent_events()
    assert len(events) == 1
    ev = events[0]
    assert ev.watch_id == wid and ev.event_type == "matched" and ev.payload_json == payload
    we_db.mark_event_sent(ev.id, sent_to=999)
    # після відправки не повинні вертатись
    assert we_db.fetch_unsent_events() == []


def test_cancel_watch_and_active_filter():
    _reset_db()
    with session_scope() as db:
        wid_keep = wp_db.create_watch(channel_id=401)
        wid_cancel = wp_db.create_watch(channel_id=402)
        db.query(m.WatchPost).filter(m.WatchPost.id == wid_cancel).update({"status": "cancelled"})
        db.commit()
        items = wp_db.list_active_watches(user_id=0)
    ids = {i[0] for i in items}
...
def test_list_active_watches_filters_statuses():
    _reset_db()
    wid_pending = wp_db.create_watch(channel_id=501)
    wid_matched = wp_db.create_watch(channel_id=502)
    wid_expired = wp_db.create_watch(channel_id=503)
    _force_status(wid_matched, "matched")
    _force_status(wid_expired, "expired")

    items_default = wp_db.list_active_watches(user_id=0)
    ...
    items_matched = wp_db.list_active_watches(user_id=0, statuses=["matched"])
...
def test_get_watch_links_or_template_links():
    ...
    with SessionLocal() as db:
        ...
        tpl_id = tpl.id
    with session_scope() as db:
        wid = wp_db.create_watch(channel_id=601, template_id=tpl_id)
