...
def _ensure_tables() -> None:
    with _conn() as c:
        ...
        # url_cache (можливо без updated_at/ts у старих БД)
        c.execute("""
        ...
        # invite_map (v2: invite, channel_id, title, updated_at)
        c.execute("""
        ...
        """)
        # Якщо БД історично зла — відновлюємо схему повністю.
        need_recreate = (
            not _has_column(c, "invite_map", "invite")
            or not _has_column(c, "invite_map", "channel_id")
            or not _has_column(c, "invite_map", "updated_at")
        )
        if need_recreate:
            c.execute("DROP TABLE IF EXISTS invite_map;")
            c.execute("""
