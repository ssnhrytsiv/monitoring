...
- **ORM/Base**: спільний `app.db.models.Base` + додаткові моделі в `app/services/models.py` (`InviteCheck`, `RequestedCheck`).  
...
- `channels`, `links`: DAO `app/DAL/channels_operations.py`; скрипти `cleanup_*`, `leave_unknown_channels.py`, `compare_db_with_sessions.py`.
...
- Для нотифікатора використовуємо спільні моделі `app.db.models` (через ORM/DAO); дубль `app/notificator_bot/db.py` видалено.
...
- admin_bot/services/channels.py: `from app.db import models as m`
- admin_bot/services/admins.py: `from app.db import models as m`
- admin_bot/services/owner_conflicts.py: `from app.db import models as m`
- admin_bot/bot/handlers/admins.py: `from app.db.session import SessionLocal`
- admin_bot/run.py: `from app.db.session import Base, engine, migrate_admins_nullable`
...
- scripts/compare_db_with_sessions.py: `from app.db.session import SessionLocal`
- scripts/compare_db_with_sessions.py: `from app.db import models as m`
- admin_bot/bot/admins_menu.py: `from app.db import models as m`
- admin_bot/bot/admins_menu.py: `from app.db.session import SessionLocal`
- admin_bot/services/subscription/refresh_channels_subscription.py: `from app.db.session import SessionLocal`
- admin_bot/services/subscription/refresh_channels_subscription.py: `from app.db import models as m`
- app/services/models.py: `from app.db.session import engine as _engine, SessionLocal, Base`
- app/services/models.py: `from app.db import models as admin_models`
- app/watch_bot/handlers/create_watch.py: `from app.db.session import SessionLocal as AdminSession`
- app/watch_bot/handlers/create_watch.py: `from app.db import models as adm_models`
